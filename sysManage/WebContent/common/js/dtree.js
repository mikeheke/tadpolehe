...
// Node object
function Node(id, pid, name, ck, url, title, target, icon, iconOpen, open) {
	this.id = id;
	this.pid = pid;
	this.name = name;
	this.url = url;
...
// Tree objectvar cbName = "roleRightVo.moduleIds";
function dTree(objName, useCB, imgPath) {
	this.config = {
		target				: null,
		folderLinks			: false,
		useSelection		: false,
...
dTree.prototype.add = function(id, pid, name, ck, url, title, target, icon, iconOpen, open) {
	this.aNodes[this.aNodes.length] = new Node(id, pid, name, ck, url, title, target, icon, iconOpen, open);
};// del a node from the node arraydTree.prototype.del= function(did,id){ 	var aNodesData = this.aNodes; 	this.aNodes=new Array(); 	for(var i=0,n=0;i<aNodesData.length ; i++){ 		if (aNodesData[i].id != id){ 			this.aNodes[n++]=aNodesData[i];		}	} 	document.getElementById(did).innerHTML=this; }; 
...
dTree.prototype.node = function(node, nodeId) {
	...
	var str = '<div id="' + this.obj+'_' + node.id + '" class="dTreeNode">' + this.indent(node, nodeId);
	...
	if(this.config.useCheckBox && nodeId!=0){
		...
		str += '<input type="checkbox" name='+cbName+' id="c'+this.parEnum+ '" ' + node.ck +' onClick="caBox(\'c'+this.parEnum+'\')" value="' + node.id +'" class="cx"/>';	}
...
/* checkbox tree */
function caBox(regx) {	
	if( document.getElementById(regx).checked ){		var dtreeCs = document.getElementsByName(cbName);
		var regxArray = regx.split("-");
		for (var i=0; i<dtreeCs.length; i++) {
			var element = dtreeCs[i];
		...
		for(j=0;j<regxArray.length-1;j++){
			if( !isCheckedByRec(regx.substring(0,regx.indexOf(regxArray[regxArray.length-j-2]+"-"))) ){
				var pDiv = regx.substring(0,regx.indexOf(regxArray[regxArray.length-j-2]+"-"));
				if(document.getElementById(pDiv))
					;//hwj delete 20111027 document.getElementById(pDiv).checked=false;
			}
		}
...
function setBroTreeV(){	//var radtree = document.getElementById('radtree');	//var rrdtree = document.getElementById('rrdtree');	var ns = new Array();	var vs = new Array();		var dtreeCs = document.getElementsByName(cbName);	for(var i=0; i<dtreeCs.length; i++) {		var element = dtreeCs[i];		if(element.type=='checkbox'){			ns[ns.length]='d_'+element.value;			vs[vs.length]=element.checked;		}	}	for(var i=0; i<ns.length; i++){		var ne = document.getElementById(ns[i]);		if(vs[i]){			ne.style.display="";		}else{			ne.style.display="none";		}	}	}